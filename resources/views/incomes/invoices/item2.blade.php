...
<div id="item-row-{{ $item_row }}" class="form-group col-md-12 well well-sm">    

    @stack('actions_td_start')
        <div class="form-group col-md-1 hidden-md hidden-lg">
            {{ trans('general.actions') }}
        </div>
        <div class="form-group col-md-1">        
            @stack('actions_button_start')
            <button type="button" onclick="$(this).tooltip('destroy'); $('#item-row-{{ $item_row }}').remove(); totalItem();" data-toggle="tooltip" title="{{ trans('general.delete') }}" class="btn btn-xs btn-danger">
    ...
    @stack('name_td_start')
        <div class="form-group col-md-4 hidden-md hidden-lg">
            {{ trans('general.name') }}
        </div>
        <div class="form-group col-md-4 {!! $errors->has('item.' . $item_row . '.name') ? 'has-error' : ''  !!}">
    ...
    @stack('quantity_td_start')
        ...
        <div class="form-group col-md-1 {!! $errors->has('item.' . $item_row . '.quantity') ? 'has-error' : '' !!}">
    ...
    @stack('price_td_start')
        <div class="form-group col-md-2 hidden-md hidden-lg">
            {{ trans($text_override['price']) }}
        </div>
        <div class="form-group col-md-2 {!! $errors->has('item.' . $item_row . 'price') ? 'has-error' : '' !!}">            
    ...
    @stack('taxes_td_start')
        <div class="form-group col-md-3 hidden-md hidden-lg">
            {{ trans_choice('general.taxes', 1) }}
        </div>
        <div class="form-group col-md-3 {!! $errors->has('item.' . $item_row . '.tax_id') ? 'has-error' : '' !!}">
